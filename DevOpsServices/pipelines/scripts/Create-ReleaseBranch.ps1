...
function Get-TeamIdFromUrl {
    ...
    try {
        ...
        Write-Warning "Could not extract team ID from URL: $TeamsUrl"
        return $null
    }
}
...
function Export-PullRequestHistoryToMarkdown {
    param (
        [Array]$PullRequests,
        [string]$OutputPath,
        [string]$Organization,
    ...
    Write-Host "##[debug]Exporting pull request history to markdown"
   
    # Define emoji codes using Unicode escape sequences
    $prEmoji = [char]::ConvertFromUtf32(0x1F4DD)      # 📝
    $userEmoji = [char]::ConvertFromUtf32(0x1F464)    # 👤
    $calendarEmoji = [char]::ConvertFromUtf32(0x1F4C5) # 📅
    $clockEmoji = [char]::ConvertFromUtf32(0x23F0)    # ⏰
    $checkEmoji = [char]::ConvertFromUtf32(0x2705)    # ✅
    $branchEmoji = [char]::ConvertFromUtf32(0x1F333)  # 🌳
    $bulletEmoji = [char]::ConvertFromUtf32(0x2022)   # •
...
"@
    ...
    foreach ($pr in $PullRequests) {
        Write-Host "##[debug]Processing PR #$($pr.pullRequestId): $($pr.title)"
        ...
        foreach ($workItem in $workItems) {
            ...
            elseif ($workItemDetails -and ($workItemDetails.fields -and $workItemDetails.fields.'System.WorkItemType' -eq 'Bug') -and ($workItemDetails.fields.'System.State' -eq 'Resolved' -or $workItemDetails.fields.'System.State' -eq 'Done' -or $workItemDetails.fields.'System.State' -eq 'Closed')) {
                $selectedPullRequests += $pr            
            }
    ...
    if ($selectedPullRequests.Count -gt 0) {
        # Calculate statistics
        $totalPRs = $selectedPullRequests.Count
        $uniqueAuthors = ($selectedPullRequests.createdBy.displayName | Select-Object -Unique).Count
       
        # Calculate average duration
        $durations = @()
        foreach ($pr in $selectedPullRequests) {
            $duration = ([DateTime]::Parse($pr.closedDate) - [DateTime]::Parse($pr.creationDate)).TotalHours
            $durations += $duration
        }
...
| Average Duration | $("{0:N1}" -f $avgDuration) hours |
| Date Range | $((Get-Date $selectedPullRequests[-1].creationDate).ToString("yyyy-MM-dd")) to $((Get-Date $selectedPullRequests[0].closedDate).ToString("yyyy-MM-dd")) |
...
"@
    ...
    # Sort pull requests by closed date (most recent first)
    $sortedPRs = $selectedPullRequests | Sort-Object -Property closedDate -Descending
    ...
    foreach ($pr in $sortedPRs) {
        ...
        $createdDate = [DateTime]::Parse($pr.creationDate).ToString("yyyy-MM-dd HH:mm:ss")
        $closedDate = [DateTime]::Parse($pr.closedDate).ToString("yyyy-MM-dd HH:mm:ss")
       
        # Calculate duration
        $duration = ([DateTime]::Parse($pr.closedDate) - [DateTime]::Parse($pr.creationDate))
        $durationStr = if ($duration.Days -gt 0) {
        ...
        $description = if ($pr.description) {
            $cleanDesc = $pr.description -replace '<[^>]+>', '' -replace '&quot;', '"' -replace '&amp;', '&'
        ...
        foreach ($reviewer in $pr.reviewers) {
            $vote = switch ($reviewer.vote) {
                10 { "$checkEmoji Approved" }
                5 { "$checkEmoji Approved with suggestions" }
                0 { "⏸ No vote" }
                -5 { " Waiting for author" }
                -10 { " Rejected" }
                default { " Unknown" }
            }
            $reviewerInfo += "$bulletEmoji $($reviewer.displayName): $vote"
        }
        $reviewersStr = if ($reviewerInfo.Count -gt 0) {
            $reviewerInfo -join "`n"
        } else {
            "_No reviewers_"
        }
        ...
        $encodedProject = Format-GitUrl -value $Project
        # Build PR URL
        $prUrl = "https://dev.azure.com/$Organization/$encodedProject/_git/$Repository/pullrequest/$prId"
       
        # Add PR details to markdown
        $markdownContent += @"
### $prEmoji $title

| Field | Value |
|-------|-------|
| **Author** | $userEmoji $author ($authorEmail) |
| **Created** | $calendarEmoji $createdDate |
| **Closed** | $calendarEmoji $closedDate |
| **Duration** | $clockEmoji $durationStr |
| **Status** | $checkEmoji Completed |
| **Source Branch** | $branchEmoji ``$sourceBranch`` |
| **Target Branch** | $branchEmoji ``$targetBranch`` |
| **Link** | [View Pull Request]($prUrl) |
...
_This document was automatically generated by Azure DevOps pipeline on $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")_
"@
    ...
    Write-Host "##[debug]Pull request history exported to $OutputPath"
...
function Format-ReleaseNotesForTeams {
    param(
        [array]$ReleaseNotes,
        [string]$Repository,
        [string]$ReleaseVersion,
    ...
    try {
        Write-Host "##[debug]Formatting $($ReleaseNotes.Count) release notes"
       
        # Create release header
        $releaseHeader = @"
$($Emojis.Rocket) **Release Notification**
...
"@
        ...
        if ($ReleaseNotes.Count -gt 0) {
            ...
            foreach ($versionGroup in $sortedGroups) {
                $version = if ($versionGroup.Name) { $versionGroup.Name } else { $ReleaseVersion }
                $releaseDate = if ($versionGroup.Group[0].ReleaseDate) {
                    $versionGroup.Group[0].ReleaseDate
                } else {
...
"@
                ...
                # Add each release note item
                foreach ($note in $versionGroup.Group) {
                    $icon = Get-ChangeTypeEmoji -ChangeType $note.ChangeType -Emojis $Emojis
                    $cleanNotes = Clean-ReleaseNoteText -ReleaseNotes $note.ReleaseNotes
                    $workItemId = if ($note.WorkItemId) { $note.WorkItemId } else { "0" }
                    
                    $versionContent += "$($Emojis.Bullet) $icon $cleanNotes (#$workItemId)`n`n"
                }
            }
        }
        else {
...
$($Emojis.Bullet) $($Emojis.Unknown) No release notes available for this version

"@
        }
       
        # Combine header with version content and footer
        $finalContent = $releaseHeader + $versionContent + "`n---`n$($Emojis.Robot) *Automated release notification from Azure DevOps*"
       
        Write-Host "##[debug]Final content length: $($finalContent.Length) characters"
       
        return $finalContent
    }
    catch {
        Write-Warning "Error formatting release notes for Teams: $_"
        return "$($Emojis.Rocket) **Release Notification**`n`nRepository: $Repository`nVersion: $ReleaseVersion`nChanges: $($ReleaseNotes.Count) updates included"
    }
}

function Get-ChangeTypeEmoji {
    param(
        [string]$ChangeType,
        [hashtable]$Emojis
    )
   
    switch -Regex ($ChangeType) {
        "Breaking Change" { return $Emojis.Breaking }
        "New Functionality" { return $Emojis.Feature }
        "Bug Fix" { return $Emojis.Fix }
        "Documentation Update" { return $Emojis.Docs }
        "Internal Optimization" { return $Emojis.Internal }
        default { return $Emojis.Unknown }
    }
}

function Clean-ReleaseNoteText {
    ...
    # Limit length for Teams message
    if ($cleanNotes.Length -gt 200) {
        $cleanNotes = $cleanNotes.Substring(0, 197) + "..."
    }
...
try {
    ...
    Write-Host "##[section]$documentEmoji Generating pull request history"
    $prHistoryPath = Join-Path $tempDir "pull_request_history_logs.md"

    if ($pullRequests.Count -gt 0) {
        Write-Host "##[debug]Exporting $($pullRequests.Count) pull requests to history file"
        $prHistoryContent = Export-PullRequestHistoryToMarkdown `
            -PullRequests $pullRequests `
            -OutputPath $prHistoryPath `
            -Organization $Organization `
            -Project $Project `
            -Repository $Repository
    
        # Commit PR history to release branch
        Write-Host "##[debug]Adding pull request history to git"
        Set-Location $tempDir
        git add $prHistoryPath
        if ($LASTEXITCODE -eq 0) {
            git commit -m "docs: Add pull request history for release $branchName"
            if ($LASTEXITCODE -eq 0) {
                git push origin $branchName
                Write-Host "##[debug]$checkEmoji Successfully committed and pushed PR history to $branchName"
            } else {
                Write-Warning "$warningEmoji Failed to push PR history, but continuing"
            }
        } else {
            Write-Warning "$warningEmoji Failed to add PR history to git, but continuing"
        }
    } else {
        Write-Host "##[debug]$infoEmoji No pull requests found, skipping PR history generation"
    }
...
"@
    ...
    # Clean up
    Write-Host "##[section]$gearEmoji Performing cleanup"
    ...
    if (Get-Location -Stack -ErrorAction SilentlyContinue) {
        Pop-Location
        Write-Host "##[debug]$checkEmoji Returned to original location"
    }
   
    Write-Host "##[debug]$checkEmoji Cleanup completed"
}
