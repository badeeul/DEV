...
function Copy-WorkItem {
    ...
    try {
            ...
            if ($sourceWorkItem.fields.PSObject.Properties.Name -contains $fieldName) {
                ...
                if ($null -ne $fieldValue) {
                    ...
                    if ($fieldName -eq "System.IterationPath" -or $fieldName -eq "System.AreaPath" -or $fieldName -eq "System.TeamProject") {
                        $fieldValue = $fieldValue -replace [regex]::Escape($SourceProjectName), $DestinationProjectName
                    }
        ...
        if ($null -ne $ParentDestinationWorkItemId -and $ParentDestinationWorkItemId -gt 0) {
            $patchDocument += @{
                ...
                value = @{
                    ...
                    url = "$OrganizationUrl/$DestinationProjectName/_apis/wit/workitems/$ParentDestinationWorkItemId"
                }
            }
        }
        ...
        $createUri = "$OrganizationUrl/$DestinationProjectName/_apis/wit/workitems/`$${encodedWorkItemType}?api-version=$script:apiVersion"
...
try {
    ...
    if ($sourceWorkItemIds.Count -eq 0) {
        throw "No work items found matching pattern: $ParentWorkItemTitle in team: $SourceTeamName"
    }
    ...
    # Check if Goal work item type exists
    $goalExists = $destinationWorkItemTypes | Where-Object { $_.name -eq "Goal" -or $_.name -eq "GOAL" }
   
    if (-not $goalExists) {
        Write-Warning "Work item type 'Goal' not found in destination project"
