...
## Installation
1. Install the required dependencies (optional if included in environment):
   ```bash
...
- **Helper Functions**:
  ...
  - `get_secret_notebookutils`: Utility function for fetching secrets from Azure Key Vault.
...
- The `notebookutils` dependency is specific to Microsoft Fabric.
