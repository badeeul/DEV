# Data Quality Rules Metadata Processor

## Overview
The `den_nbk_pdi_001_cicd_dq_rules_processor` is a Microsoft Fabric notebook designed to processes data quality rules metadata from an Excel file, convert them to JSON format, and write the results to a Delta Lake table. It is designed to run in a Microsoft Fabric environment with Spark and OneLake integration. The script includes row count validation, custom package verification, and comprehensive error handling.
...
2. Place (manual or cicd) the Excel file in the appropriate OneLake directory.
3. The notebook will be **automatically run as part of the CI/CD process** if its name contains the string `_cicd_`.
4. Run the script in a Fabric notebook or Spark environment:

```python
# For full execution
main(run_me=True)

# To skip execution (useful for testing configuration and/or during deployment)
main(run_me=False)
```
